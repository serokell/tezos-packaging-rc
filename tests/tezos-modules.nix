...
import "${nixpkgs}/nixos/tests/make-test-python.nix" ({ ... }:
{
  ...
  machine = { ... }: {
    ...
    services = {
      octez-node.instances.nairobinet = octez-node;
      octez-signer.instances.nairobinet = octez-signer;
      octez-accuser.instances.nairobinet = octez-accuser;
      octez-baker.instances.nairobinet = octez-baker;
    };
  ...
  testScript = ''
    ...
    for s in services:
        machine.wait_for_unit(f"tezos-nairobinet-{s}.service")
    ...
    with subtest("service status sanity check"):
        for s in services:
            machine.succeed(f"systemctl status tezos-nairobinet-{s}.service")
  '';
}) { inherit pkgs system; }
