...
while : ; do
  ...
  git rebase --exec 'git commit --amend -n --gpg-sign="tezos-packaging@serokell.io" -m "$(git show -s --format=%s | sed -e "s/ <unsigned>//g")"' \
    "origin/$BUILDKITE_PIPELINE_DEFAULT_BRANCH" || (git rebase --abort; exit 1)
