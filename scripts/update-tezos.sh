...
if [[ "$latest_upstream_tag" != "$our_tezos_tag" ]]; then
  ...
  if ! git rev-parse --verify "$branch_name"; then
    ...
    nix flake lock --override-input tezos "gitlab:tezos/tezos/$latest_upstream_tag_hash"
    nix flake lock --override-input opam-repository "gitlab:tezos/opam-repository/$opam_repository_tag"
