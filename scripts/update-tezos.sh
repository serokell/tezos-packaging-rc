...
git config user.name "Savely Krendelhoff" # necessary for pushing
git config user.email "alicecandyom@proton.me"
git fetch --all

# Get latest tag from tezos/tezos
git clone https://gitlab.com/tezos/tezos.git upstream-repo
cd upstream-repo
latest_upstream_tag_hash="f59e1b0eaa26e97a23824dc4fbd4af2bd8fcf8af"
...
if [[ "$latest_upstream_tag" != "$our_tezos_tag" ]]; then
  ...
  if ! git rev-parse --verify "$branch_name"; then
    ...
    git commit -a -m "[Chore] Bump Tezos sources to $latest_upstream_tag" --gpg-sign="alicecandyom@proton.me"

    ./scripts/update-brew-formulae.sh "$latest_upstream_tag-1"
    git commit -a -m "[Chore] Update brew formulae for $latest_upstream_tag" --gpg-sign="alicecandyom@proton.me"
    ...
    git commit -a -m "[Chore] Reset release number for $latest_upstream_tag" --gpg-sign="alicecandyom@proton.me" || \
    ...
    git commit -a -m "[Chore] Reset letter_version for $latest_upstream_tag" --gpg-sign="alicecandyom@proton.me" || \
